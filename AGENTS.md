...
- 如无明确错误迹象、用户要求或既定实验协议要求，不做防御性编程和冗余验证。
  无论文件大小，生成后都不为“保险”追加哈希、重新读取、解压、遍历或其他验证；
  达到可用状态就立即交付。排错只针对实际问题做最小范围检查。
...
- 紧凑原始指标、metadata 和 gate JSON 可按 run_id 随实验 RECORD 保存原样副本，
  ...
  也不改变数据、权重、完整日志和 secrets 不进入 Git 的边界。
...
- 给用户的服务器代码块必须自动从 `id -un` 识别 lab0/lab1/lab2，并自动解析 GPU、bundle
...
- `lab0` 只用物理 GPU 0，`lab1` 只用物理 GPU 1，`lab2` 只用物理 GPU 2；启动前检查 GPU 占用。
