...
# and checkpoint requirements live in the EXP025 resource gate below.
#
...
require_exp025_resources() {
    local config="$1" hierarchy initialization arm expected_machine
    ...
    arm="$(container_config_value "${config}" EXP025_ARM)" || fail "cannot read EXP025 arm"
    arm="${arm##*$'\n'}"
    case "${arm}" in
        official_frozen) expected_machine=lab0 ;;
        imagenet_full) expected_machine=lab1 ;;
        *) fail "unknown EXP025_ARM: ${arm}" ;;
    esac
    [[ "${machine}" == "${expected_machine}" ]] || \
        fail "EXP025 arm ${arm} must run on ${expected_machine}, got ${machine}"
...
launch() {
    local mode="$1" config="$2" checkpoint_container="${3:-}" gate_scale="${4:-}" run_id run_host run_container
...
main() {
    ...
    gate)
        ...
        validate_experiment_id "${experiment_id}"
        config="$(resolve_config "$2")"
        [[ "$3" =~ ^[0-9]+([.][0-9]+)?$ ]] && awk -v scale="$3" 'BEGIN { exit !(scale >= 1) }' || \
            fail "AMP scale must be at least 1: $3"
        launch gate "${config}" "" "$3"
        ;;
    eval)
        [[ $# -eq 3 ]] || usage
        experiment_id="$1"
