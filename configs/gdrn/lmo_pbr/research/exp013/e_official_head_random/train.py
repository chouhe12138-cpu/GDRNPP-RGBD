...
# from EXP013A/B/C. The head is wrapped (OfficialConvPnPNetRandomInit) so its
# state-dict keys live under pnp_net.head.* and can never collide with the
# official checkpoint's pnp_net.* keys: MODEL.WEIGHTS stays the original
# official checkpoint and the random initialization is preserved (identical
# mechanism to the EXP013A/B/C key-mismatch protection). The head flags
# replicate the official configuration, NOT the EXP013 family: no mask
# multiplicative gating (MASK_ATTENTION="none"), GN+gelu, flatten fc1.
...
MODEL = dict(
    ...
    POSE_NET=dict(
        ...
        PNP_NET=dict(
            ...
            INIT_CFG=dict(
                _delete_=True,
                type="OfficialConvPnPNetRandomInit",
                norm="GN",
                act="gelu",
                num_gn_groups=32,
                drop_prob=0.0,
