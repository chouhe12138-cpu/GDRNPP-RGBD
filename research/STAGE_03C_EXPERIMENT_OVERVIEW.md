...
本文件保留 Stage 3C 及其向 Stage 4 衔接时的阶段快照，用于追溯当时的比较、
门槛和结果，不承担整个项目的当前状态。当前研究状态见 `research/STATUS_CN.md`，
完整实验列表见 `research/EXPERIMENT_INDEX.md`。正式协议和证据链接见
[实验记录与协议](#实验记录与协议)。

后续实验推进不会要求反向改写本页的历史结果；其中“当前”语义均以本快照日期为准。
