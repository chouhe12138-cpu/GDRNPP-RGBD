...
（`lm13` / `lm13_pbr` / `lmo_full_imagenet` / `legacy_lmo`，见 `RUNBOOK_CN.md`）。
LM13 两档另外在容器里跑 `research.exp022.server_preflight`；LM-O ImageNet 全训练档
在容器里跑 `research.exp022.preflight`。任一项失败都不会启动训练/评估，也不会创建输出目录。
