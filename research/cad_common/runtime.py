"""Dataset-neutral CAD batch, AMP, and local diagnostic helpers."""
...
def real_batch(cfg, device, batch_size, renderer_type, load_batch=None, save_batch=None,
    ...
    if load_batch:
        ...
        if ('dataset_key' not in batch and not allow_legacy_unlabeled) or (
                'dataset_key' in batch and batch['dataset_key'] != context.key) or (
                tuple(batch.get('object_ids', context.object_ids)) != context.object_ids):
