...
| EXP023 LM13 Progressive PCC Fulltrain | PROTOCOL_OVERHAULED / LOCAL_TRAIN_AND_DATA_CHECK_PASS / FORMAL_NOT_STARTED | 按 GDR-Net LM 协议拆出 `lm13_gdrn`/`real_only`/`pbr` 三条显式协议，LM real 与 lm_imgn loader、增强语义、preflight 协议校验、评估 manifest 已接入；本机数据数值检查与真实训练入口短训练通过。**两条评估链只能用 GT bbox**（官方检测框缺失），无精度结论，正式训练未开始 | [RECORD](experiments/EXP-20260918-023-lm13-progressive-pcc-fulltrain/RECORD.md) |
