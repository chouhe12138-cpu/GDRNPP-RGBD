...
| EXP023 LM13 Progressive PCC Fulltrain | PRETRAIN_CHECKS_PASS / EGL_SMOKE_BLOCKED / FORMAL_NOT_STARTED | 三条显式协议、两个 loader、preflight 硬校验、评估 manifest 与训练前收口完成（`TARGET_LR_FACTOR=0.0`、test split 去 `xyz_crop`、`EVAL_PERIOD=20`）；preflight/数据检查/CPP smoke/真实入口 1 epoch + checkpoint 均通过，`248 passed`。本机 EGL 不可用（`Bindless Textures not supported`），**NO-GO**，唯一 blocker 是服务器 EGL smoke | [RECORD](experiments/EXP-20260918-023-lm13-progressive-pcc-fulltrain/RECORD.md) |
