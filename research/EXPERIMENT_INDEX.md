...
| EXP023 LM13 Progressive PCC Fulltrain | SERVER_INTEGRATION_ADDED / EGL_SMOKE_PENDING / FORMAL_NOT_STARTED | 协议重整、训练前收口与服务器集成完成：三协议 + 两 loader + preflight 硬校验 + `lm_imgn` 只读 mount + ConvNeXt env + profile-aware runtime gate + `server_preflight`。本机数据/CPP smoke/真实入口 1 epoch + checkpoint 通过，`275 passed`。本机 EGL 不可用；**服务器侧的挂载、容器 env 与 EGL smoke 未验证**，`FORMAL_READY` 仍为 `False` | [RECORD](experiments/EXP-20260918-023-lm13-progressive-pcc-fulltrain/RECORD.md) |
