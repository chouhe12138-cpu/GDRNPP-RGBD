...
本文件保留 EXP007 姿态头信息流诊断在该日期的协议与交接上下文，不承担整个项目
的当前状态。当前研究摘要见 `research/STATUS_CN.md`，实验状态见
`research/EXPERIMENT_INDEX.md` 和 EXP007 的具体实验记录。本文中的运行状态、
操作约束和“当前”措辞均按本快照日期理解。
...
本诊断执行时的操作约束是：开始工作前先运行 `git status`，保留全部未提交内容；
当时 C1 正式训练必须完成预注册的 40 轮，本诊断不得停止、替换或干扰该训练。
